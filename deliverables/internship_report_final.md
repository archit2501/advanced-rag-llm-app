...
*(This page is the cover page. Replace the bracketed fields with your details and, if your department provides a cover-page template, use it instead. Apply your department's alignment and logo.)*
...
A Summer / Industry Internship Report
...
Faculty Guide: [Faculty Supervisor Name]

Industry Guide: [Mentor Name], [Host Organisation]
...
*This page reproduces the internship offer letter issued by the organisation. In the final submission, replace it with the original signed letter on the organisation's letterhead.*

\vspace{0.6cm}

**[HOST ORGANISATION]**

[Registered address, City, PIN]

\vspace{0.4cm}

Ref: [HR/Internship reference number]  \hfill  Date: 5 May 2026
...
| Project area | Retrieval-augmented generation over a document collection |
| Duration | 19 May 2026 to 4 July 2026 (seven weeks) |
| Reporting to | [Mentor Name], Industry Guide |
| Location | [Office location / remote] |
| Stipend | [As applicable] |

During the internship you will be expected to follow the organisation's code of conduct and data-handling policy, and to maintain the confidentiality of any material shared with you. This offer is for a learning internship and does not constitute an offer of employment.
...
*This page reproduces the internship completion certificate issued by the organisation. In the final submission, replace it with the original certificate on the organisation's letterhead.*
...
Ref: [Certificate reference number]  \hfill  Date: 4 July 2026
...
We wish the intern success in future endeavours.
...
*A short synopsis of the internship project, to be signed by the student, the industry guide, and the faculty guide.*

**Title:** Building a Grounded Retrieval-Augmented Question Answering System over a Controlled Document Collection.

**Problem.** A language model does not know the contents of a specific document set, and it will answer a question even when nothing in those documents supports an answer. Both behaviours are a problem for an assistant meant to look up rules and facts from a fixed collection.

**Objective.** Build a question answering system that retrieves relevant passages from a fixed document collection, answers from those passages with citations back to the source, and refuses when the retrieved evidence is too weak. The system should run offline by default so that it is reproducible, and every stage should be measurable on its own.

**Approach.** The system ingests documents and splits them into passages, retrieves the most relevant passages using keyword ranking (BM25) and vector similarity fused with Reciprocal Rank Fusion, checks the evidence with a gate before answering, and generates an answer with inline citations through a choice of an offline extractive answerer, the OpenAI Responses API, or a local Ollama model. It is a FastAPI service over SQLite with a small web interface, and it carries a versioned test set of 41 questions for evaluation.

**Outcome.** On the test set the offline configuration reached an average required-fact recall of 0.8241 and correctly refused four of five unanswerable questions. A later experiment swapped the deterministic embedder for a learned sentence-embedding model; this improved retrieval ranking but, on this corpus, did not improve the answers and broke the refusal thresholds, which had been tuned to the earlier score scale. The recurring theme of the work was measuring each stage separately and reporting the results honestly, including where they were weaker than hoped.
...
*In the editable (Word) version, you may regenerate this table from the heading styles using the word processor's References menu.*
...
| Abbreviations | 12 |
| 1. Introduction | 13 |
| 2. Requirement Specification and Preliminaries | 16 |
| 3. Tasks Accomplished in the Organization | 31 |
| 4. Results and Outcome | 48 |
| 5. Conclusion | 55 |
| References | 59 |
| Plagiarism Report | 60 |
| Weekly Progress Reports (WPRs) | 61 |
| Daily Diary | 66 |
| Appendix A. Configuration reference | 70 |
| Appendix B. Command reference | 72 |
| Appendix C. Glossary | 73 |
| Appendix D. Repository and artifacts | 74 |

\newpage

# List of Figures

The figures in this report are text diagrams within the Tasks Accomplished section. Page numbers refer to the print layout and should be regenerated if the document is re-typeset.

| Figure | Title |
| --- | --- |
| Figure 1 (H.1) | The modules and how a request moves through them |
| Figure 2 (H.2) | The document ingestion flow |
| Figure 3 (H.3) | The question answering flow |
...
| Table 3 | Answer-evaluation results, default (hashing) embedder |
| Table 4 | Retrieval comparison, default (hashing) embedder |
| Table 5 | Retrieval comparison, learned embedder |
| Table 6 | Configuration and environment variables |
| Table 7 | Weekly progress summary |
...
# 2. Requirement Specification and Preliminaries

This section gives the background concepts the project rests on, the tools used, the system design, and the requirements the system was built to meet. A reader familiar with retrieval-augmented generation can move quickly through the first parts.
...
### Storage: an honest note on the word "index"
...
# 3. Tasks Accomplished in the Organization

This section describes the work carried out during the internship: the week-by-week account of what was built, the implementation of each part of the system, selected code, and the system diagrams. The formal weekly progress reports and the daily diary appear near the end of the report.
...
### Week 1 (19–23 May): scoping and reading
...
### Week 2 (26–30 May): ingestion and storage
...
### Week 4 (9–13 June): generation, providers, and the evidence gate
...
### Week 5 (16–20 June): the API, the interface, and the test set
...
### Week 6 (23–27 June): comparison, and a code review that found real problems
...
### Week 7 (30 June–4 July): the learned-embedding experiment, and finishing
...
This section reports what the system does on the test set, the problems encountered and what they taught me, and a worked example of the retrieval metrics.
...
### Results with the default embedder
...
### Results with a learned embedder
...
### The vector search that was not semantic
...
### The general shape of these lessons
...
### Sending documents to an external service
...
### The ethics of a confident assistant
...
*This page is a plagiarism declaration and report summary. Run your institution's similarity-check tool (for example the one provided by your department), enter the resulting similarity index below, attach the tool's report, and sign.*

\vspace{0.6cm}

**PLAGIARISM DECLARATION AND REPORT**
...
| Item | Detail |
| --- | --- |
| Tool used | [Name of similarity-check tool] |
| Date of check | __________ |
| Similarity index | ________ % |
| Report reference | [Attach the tool's summary report] |
...
Each week I submitted a short progress report to my industry guide. They are reproduced below in a common format. Each was reviewed in the Friday meeting.
...
| --- | --- |
| Week number | 1 |
| Period | 19 to 23 May 2026 |
| Objective for the week | Scope the project and set up the environment. |
| Work carried out | Read the RAG and BM25 background. Chose the technology stack and the offline-first approach. Set up the repository, the virtual environment, and the folder structure. Collected the sample documents and defined the eight objectives. |
| Outcome and deliverables | A defensible one-sentence problem statement, the objective list, and a working project skeleton. |
| Industry guide's remarks | ________________________________________ |
...
| --- | --- |
| Week number | 2 |
| Period | 26 to 30 May 2026 |
| Objective for the week | Build document ingestion and storage. |
| Work carried out | Wrote text extraction and normalisation, the paragraph-packing chunker with overlap and the fix for over-long paragraphs, content-derived document identifiers, and the SQLite schema and insert logic. Added a chunking test. |
| Outcome and deliverables | Documents can be ingested, chunked, and persisted; ingestion is idempotent. |
| Industry guide's remarks | ________________________________________ |
...
| --- | --- |
| Week number | 3 |
| Period | 2 to 6 June 2026 |
| Objective for the week | Build retrieval. |
| Work carried out | Wrote the hashing embedder, the tokeniser, the BM25 scorer, and the Reciprocal Rank Fusion. Discovered that the hashing vector signal is lexical rather than semantic and recorded that honestly. Added per-result diagnostics. |
| Outcome and deliverables | A working hybrid retriever with full diagnostics, and a clear understanding of the embedder's limits. |
| Industry guide's remarks | ________________________________________ |
...
| --- | --- |
| Week number | 4 |
| Period | 9 to 13 June 2026 |
| Objective for the week | Build generation, providers, and the evidence gate. |
| Work carried out | Wrote the offline extractive answerer and the OpenAI and Ollama adapters behind one interface, the grounded prompt, the citation validation, and the multi-signal evidence gate. Tuned the gate thresholds on the corpus and noted they were not universal. |
| Outcome and deliverables | Answers with citations from any of three providers, and a refusal path for weak evidence. |
| Industry guide's remarks | ________________________________________ |
...
| --- | --- |
| Week number | 5 |
| Period | 16 to 20 June 2026 |
| Objective for the week | Build the API, the interface, and the test set. |
| Work carried out | Wrote the FastAPI endpoints and the web interface, built the 41-question test set with answerable, ambiguous, conflicting, and unanswerable cases, wrote the evaluation runner, and added the browser test. Ran the first full evaluation. |
| Outcome and deliverables | A usable interface and the first measured results, with their limitations recorded. |
| Industry guide's remarks | ________________________________________ |
...
| --- | --- |
| Week number | 6 |
| Period | 23 to 27 June 2026 |
| Objective for the week | Compare retrieval strategies and act on a code review. |
| Work carried out | Wrote the vector, keyword, and hybrid comparison with Recall, MRR, and nDCG. Received a code review that found the fabricated citation, an unsafe cross-origin policy, dead code, a made-up model name, an unbounded upload, and unprotected endpoints. Fixed all of them. |
| Outcome and deliverables | A fair retrieval comparison and a materially more correct and honest codebase. |
| Industry guide's remarks | ________________________________________ |
...
| --- | --- |
| Week number | 7 |
| Period | 30 June to 4 July 2026 |
| Objective for the week | Run the learned-embedding experiment and finish. |
| Work carried out | Added the sentence-transformer embedder behind the existing interface and ran the ablation. Found that learned embeddings improved ranking but not the answers on this corpus and broke the uncalibrated gate. Wrote the documentation and this report, corrected stale documentation, and published the repository. |
| Outcome and deliverables | The completed system, the ablation result and its explanation, and the final documentation. |
| Industry guide's remarks | ________________________________________ |
